...
namespace DVT.Core.Helper
{
        ...
        , IComparable<BigDecimal>
    {
        ...
        {
            string textBeforeTheExponent = "";
            int actualExponent = 0;
            int derivedExponent = 0;

            bool isDecimal = value.Contains(".");
            bool hasExponent = value.IndexOf("e", StringComparison.OrdinalIgnoreCase) != -1;
                        
            //Regular decimal number
            if (isDecimal && !hasExponent)
            {
                derivedExponent = value.Length - (value.IndexOf(".") + 1);
                BigInteger mantissa = BigInteger.Parse(value.Replace(".", ""));
                return new BigDecimal(mantissa, -derivedExponent);
            }
                        
            if (hasExponent)
            {
                var parts = value.Split(new char[] { 'e', 'E' }, StringSplitOptions.RemoveEmptyEntries);
                
                if (parts.Length == 2)
                {            
                    textBeforeTheExponent = parts[0];
                    int.TryParse(parts[1], out actualExponent);                   

                    if(isDecimal)
                    {
                        derivedExponent = textBeforeTheExponent.Length - (textBeforeTheExponent.IndexOf(".") + 1);

                        //Adjust the derived exponent by adding the exponent part from the number i.e. -5 in 2.2e-005
                        derivedExponent = actualExponent - derivedExponent;

                        BigInteger mantissa = BigInteger.Parse(textBeforeTheExponent.Replace(".", ""));
                        return new BigDecimal(mantissa, derivedExponent);
                    }
                    else
                    {
                        BigInteger mantissa = BigInteger.Parse(textBeforeTheExponent);
                        return new BigDecimal(mantissa, actualExponent);
                    }
                }
            }            
